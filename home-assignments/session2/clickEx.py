...
@click.option('-c/-f', default=True, help='temparture in celsius or fahrenheit')

def main(city, forecast, c):
    check_city_name(city)
    ...
    if forecast == "TODAY":
        low_temperature, high_temperature = output_for_single_day(forecast, location)
...
def check_city_name(city):
    if city == "Error":
        print("use the --city flag followed by a city name")
        sys.exit()
...
def check_forecast_input(forecast):
    ...
    if len(forecast) == 6 and forecast[5] != "+":
        print("Input incorrect!, forecast parameter should start with TODAY, TODAY+int can be used to receive upcoming forecasts")
        sys.exit()
    if len(forecast) <= 5:
        return
    else:
...
def what_temperature_metric(c):
    if c:
        ...
        temparture_metric = "celsius"
    else:
        weather = weather_in_fahrenheit()
        temparture_metric = "fahrenheit"
    return temparture_metric, weather
...
def output_for_single_day(forecast, location):
    forecasts = location.forecast
    return forecasts[0].low, forecasts[0].high
